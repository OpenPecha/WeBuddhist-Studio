...
const mockPlanData = {
  ...
  days: {
    day1: { label: "Day 1", tasks: ["Morning Intention Setting", "Compassion Reflection"] },
...
describe("PlanDetailsPanel Component", () => {
  ...
  it("shows tasks when a day is selected and expanded", () => {
    render(<PlanDetailsPanel />);
    expect(screen.getByText(mockPlanData.days.day1.tasks[0])).toBeInTheDocument();
    expect(screen.getByText(mockPlanData.days.day1.tasks[1])).toBeInTheDocument();
    const day2 = screen.getByText(mockPlanData.days.day2.label);
    fireEvent.click(day2);
    expect(screen.getByText(mockPlanData.days.day2.tasks[0])).toBeInTheDocument();
    expect(screen.queryByText(mockPlanData.days.day1.tasks[0])).not.toBeInTheDocument();
  });
