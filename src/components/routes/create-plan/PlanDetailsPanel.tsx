...
const PlanDetailsPanel = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <div className="w-80 bg-white border-r border-t border-gray-200 h-full flex flex-col">
        <div className="p-4">
          <div className="text-red-600 text-md font-semibold mb-1">
            Current Plan
          </div>
        ...
        <div className="flex-1 p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-md font-semibold text-gray-700">Days</span>
          </div>

          <div className="space-y-2">
            {mockPlanData.days.map((day) => (
                ...
                >
                  ...
                  {selectedDay === day.day_number && (
                    <div className="flex items-center gap-2">
                      <IoMdAdd className="w-4 h-4 text-gray-400 cursor-pointer" />
                      <MdExpandMore
                  ...
                  day.tasks.length > 0 && (
                    <div className="ml-9 mt-2 space-y-1">
                      {day.tasks.map((task) => (
                        <div
                          key={task.id}
                          className="flex items-center justify-between py-2 px-3 text-sm text-gray-700 bg-gray-50 rounded"
                        >
                          <span>{task.title}</span>
                          <FiTrash className="w-3 h-3 text-gray-400 cursor-pointer" />
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <div className="mt-4">
            <div className="flex items-center gap-2 py-2 px-3 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
              <IoMdAdd className="w-4 h-4 text-gray-700" />
              <span className="text-md font-medium text-gray-700">
                Add New Day
              </span>
            </div>
