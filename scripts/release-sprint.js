  ...
  releaseSprint(tagType = "sprint", pushChanges = true) {
    ...
    if (pushChanges) {
      try {
        ...
        console.log(
          `⚠️  Failed to push to ${currentBranch} (possibly protected branch)`,
        );
    ...
    console.log(
      `🎉 ${tagType.charAt(0).toUpperCase() + tagType.slice(1)} release ${version} completed successfully!`,
    );
    return version;
  }
}
...
function main() {
  ...
  const tagType = args.includes("--type")
  ...
  if (!validTypes.includes(tagType)) {
    console.error(
      `❌ Invalid tag type: ${tagType}. Must be one of: ${validTypes.join(", ")}`,
    );
    process.exit(1);
  }
